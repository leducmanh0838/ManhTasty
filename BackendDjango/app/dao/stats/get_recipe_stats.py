...
def get_recipe_stats(by="month", start_date=None, end_date=None):
    """
    Thống kê số công thức theo khoảng thời gian (day, month, quarter, year).

    - by: "day" | "month" | "quarter" | "year"
    - start_date: ngày bắt đầu (datetime.date hoặc datetime)
    - end_date: ngày kết thúc (datetime.date hoặc datetime)
    """

    qs = Recipe.objects.all()

    # Nếu có khoảng ngày thì filter
    if start_date:
